...
// ─── Popup UI: combined login + register with tab switching ───

export async function showAuthPopup(initialTab = "login") {
  let tab = initialTab
  let result = null
  ...
  while (!result) {
    ...
    if (!username) {
      ...
      // Clear the popup (move up ~14 lines)
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (tab === "login" && (data.error || "").includes("Invalid")) {
      ...
      await saveAuthToken(data.token, data.user)
      result = data
    } catch (err) {
      console.log("")
  ...
  console.log(boxEmpty())
  console.log(boxRow(`  ${chalk.green("✓")} Logged in as ${chalk.white.bold(result.user.username)}`, `  ✓ Logged in as ${result.user.username}`))
  console.log(boxEmpty())
...
// ─── Legacy direct handlers (still usable as sys login / sys register) ───
  ...
  if (!user || !token) {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        console.log("")
        console.log(chalk.dim(`  Plan: ${chalk.white(data.planLabel || data.plan)}`))
        if (data.plan === "free") {
